...
1. **Critério X falhou em N de M últimas specs** (N ≥ 2 e M ≤ 5) → propor uma de:
   ...
   - `[CRÍTICO]` passo novo na skill que produz aquele tipo de artefato (se o critério revela passo faltando em `spec-and-plan` ou no gate de ship)
...
4. **Trace de `--fresh` sempre invocado** num tipo de spec → propor `[OTIMIZAÇÃO]` mover `--fresh` pra default daquele rubric (mudar threshold de invocação no gate de ship).
